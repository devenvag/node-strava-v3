...
function parseRateLimits(res) {
    if(res === undefined || !res.headers[util.rateLimit] || !res.headers[util.rateUsage]) {
        return null;
    }

    var limit = res.headers[util.rateLimit].split(',')
        , usage = res.headers[util.rateUsage].split(',')
        , radix = 10;
